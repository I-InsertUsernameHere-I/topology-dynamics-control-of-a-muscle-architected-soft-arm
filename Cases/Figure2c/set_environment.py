...
class Environment:
    ...
    def reset(self):
        ...
        for idx, rod in enumerate(self.ring_rod_list[:]):
            if idx < 60:
                self.simulator.add_forcing_to(rod).using(
                    SigmoidActivationTransverseMuscles,
                    rod_idx=idx,
                    ...
                    beta=1,
                    tau=0.01,
                    activation_level_max=0.4,
                    activation_level_end=0.4,
                )  # 1.0
            elif idx >= 60 and idx <= 160:
                self.simulator.add_forcing_to(rod).using(
                    SigmoidActivationTransverseMuscles,
                    rod_idx=idx,
                    start_time=7.0,
                ...
                self.simulator.add_forcing_to(rod).using(
                    ...
                    beta=1,
                    tau=0.01,
                    activation_level_max=0.03,
                    activation_level_end=0.03,
                )  # 0.5
